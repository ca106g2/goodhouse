						...
						<div class="card-body">
							<form method="post" action="ele_contract.do" name="form1">
								<div>
									...
									第三條︰租賃期間︰共<b><input type="text" name="ele_rent_time"
										value="${eleConVO.ele_rent_time} " class="btn btn-light" /></b>個月
									（即<b><input type="text" name="ele_rent_f_day"
										id="ele_rent_f_day" value="${eleConVO.ele_rent_f_day}"
										class="btn btn-light" /></b>起 至<b><input type="text"
										name="ele_rent_l_day" id="ele_rent_l_day"
										value="${eleConVO.ele_rent_l_day}" class="btn btn-light" /></b>止）
									期滿乙方應即無條件遷還房屋不得提出任何要求獲條件。乙方並應依規定申報戶口（包括流動戶口）。<br>
									第四條︰房租︰每月新台幣 <input type="text" name="ele_rent_money"
										value="${eleConVO.ele_rent_money}" class="btn btn-light" />元，
									<c:forEach var="bill_paymenttype"
										items="${Bill_PaymentTypeMap}">
										<c:if
											test="${eleConVO.bill_paymenttype eq bill_paymenttype.key}">
											<p>
												<b>${bill_paymenttype.value.type_name}</b>
											</p>
										</c:if>
									</c:forEach>
									方式繳交不得拖欠。<br> 第五條：押金新台幣 <input type="text"
										name="ele_deposit_money" value="${eleConVO.ele_deposit_money}"
										class="btn btn-light" />
									...
									<div class="text-center">
										<p>合約備註</p>
										<textarea name="ele_con_note" rows="3" cols="30"
											value="${eleConVO.ele_con_note}" class="btn btn-light"></textarea>
									</div>

									<div  style="font-size: 20px;" class="text-center">
										<br> 立契約書人 甲 方：<b>${memSvc.getOneMem(lanSvc.getOneLan(eleConVO.lan_id).mem_id).mem_name}</b><br>
										房東身分證字號：<input type="text" name="lan_idnumber"
											value="${eleConVO.lan_idnumber}" class="btn btn-light" /><br>

										乙 方：<b>${memSvc.getOneMem(eleConVO.mem_id).mem_name}</b><br>
										房客身份證字號：<input type="text" name="mem_idnumber"
											value="${eleConVO.mem_idnumber}" class="btn btn-light" /><br>

										簽約日：<input type="text" name="ele_singdate" id="ele_singdate"
...
$(function(){
	...
	$('#ele_singdate').datetimepicker({
		   ...
		   step: 1,                //step: 60 (這是timepicker的預設間隔60分鐘)
		   format:'Y-m-d',         //format:'Y-m-d H:i:s',
		   value: '<%=eleConVO.getEle_singdate()%>
		', // value:   new Date(),
				//disabledDates:        ['2017/06/08','2017/06/09','2017/06/10'], // 去除特定不含
				//startDate:	            '2017/07/10',  // 起始日
				minDate : '-1970-01-01', // 去除今日(不含)之前
