	...
	<div class="row justfy-content-center " >
			...
			<div>
				...
				<div style="font-size:20px">
					...
					承租人       <b><input type="text" id="mem_name" name="mem_name" class="btn btn-light" value="${memSvc.getOneMem(eleConVO.mem_id).mem_name}" placeholder="輸入房客姓名" /></b> 
					（以下簡稱乙方），茲為房屋一部租賃、雙方議定契約條款如下：<br>
					<br>
					第一條︰租賃房屋地址︰
					...
					<label for="exampleFormControlSelect1"></label>
						...
						<select size="1" name="hou_id" style="overflow:hidden; text-overflow:ellipsis;white-space:nowrap;width:210px;" 
							class="form-control " id="exampleFormControlSelect1" data-toggle="tooltip" data-placement="right" title="沒有房屋記得新增房屋喔!!">
							<c:forEach var="houVO" items="${houSvc.all}">
								<c:if test="${houVO.lan_id eq lanSvc.getOneLanByMemId(memVO.mem_id).lan_id }">
									<option id="hou" value="${houVO.hou_id}" class=" form-control btn btn-light" />
										${houVO.hou_address}
								</c:if>
							</c:forEach>
						</select><br>
					...
					<br>
					第三條︰租賃期間︰共<b><input type="text" name="ele_rent_time" value="${eleConVO.ele_rent_time}" class="btn btn-light" placeholder="輸入總租期(共幾個月)"/></b>個月
					...
					<br>
					第四條︰房租︰每月新台幣  <b id="ele_rent_money"></b> 元，${Bill_PaymentTypeMap['p1'].type_name}方式繳交不得拖欠。<br>
<%-- 					第四條︰房租︰每月新台幣  <b><input type="text" name="ele_rent_money" id="ele_rent_money" value="${eleConVO.ele_rent_money}" class="btn btn-light" placeholder="輸入租金"/></b> 元，${Bill_PaymentTypeMap['p1'].type_name}方式繳交不得拖欠。<br> --%>
					<br>
					第五條：押金新台幣  <b><input type="text" name="ele_deposit_money" id="ele_deposit_money" value="${eleConVO.ele_deposit_money}" class="btn btn-light" placeholder="輸入押金"/> </b> 元。尤乙方於訂約時交付甲方收取保管，租賃期滿乙方依約遷還房屋時，由甲方無息發還，如有乙方應付甲方之款未付清時由此扣款還乙方不得異議。又租賃期間未滿時不得以任何理由請求退還押租金。<br>
					<br>
					第六條：特約事項︰<br>
					...
					<br>
					<p>合約備註</p>
					<textarea name="ele_con_note" rows="3" cols="50" value="${eleConVO.ele_con_note}" class="btn btn-light" placeholder="輸入其他特別協議">${eleConVO.ele_con_note}</textarea><br>

				</div>
				
				<div style="font-size:20px;" class="text-center">
					立契約書人  甲            方：<b>${memVO.mem_name}</b><br>
							身分證字號：<b><input type="text" name="lan_idnumber" value="${eleConVO.lan_idnumber}" class="btn btn-light" placeholder="輸入房東身分證字號"/></b><br>
...
<%-- 					乙            方：<b><input type="text" id="mem_name2" name="mem_name" class="btn btn-light" value="${memSvc.getOneMem(eleConVO.mem_id).mem_name}"/></b><br> --%>
					...
					身份證字號：<b><input type="text" name="mem_idnumber" value="${eleConVO.mem_idnumber}" class="btn btn-light" placeholder="輸入房客身份證字號"/></b><br>
