					...
					<form method="post" action="ele_contract.do" name="form1">
						...
						<div style="font-size: 20px">
							...
							<div class="form-group">
								<label for="exampleFormControlSelect1"> <select size="1"
									name="hou_id"
							...
							<br> 第三條︰租賃期間︰共<b><input type="text"
								name="ele_rent_time" value="${eleConVO.ele_rent_time}"
								class="btn btn-light" placeholder="輸入總租期(共幾個月)" /></b>個月 （即<b><input
								type="text" name="ele_rent_f_day" id="ele_rent_f_day"
								value="${eleConVO.ele_rent_f_day}" class="btn btn-light" /></b>起 至<b><input
								type="text" name="ele_rent_l_day" id="ele_rent_l_day"
								value="${eleConVO.ele_rent_l_day}" class="btn btn-light" /></b>止）期滿乙方應即無條件遷還房屋不得提出任何要求獲條件。乙方並應依規定申報戶口（包括流動戶口）。<br>
							<br> 第四條︰房租︰每月新台幣 <b id="ele_rent_money"></b>
							...
							<br> 第五條：押金新台幣 <b><input type="text"
								name="ele_deposit_money" id="ele_deposit_money"
							...
							<br> 二、租用期間應繳納之政府稅捐由甲方負擔，但每月水電費由乙方負擔與其他房客分攤，繳納後收據交由甲方保存。<br>
							<br>
							三、租賃期間原有設施及水電衛生設備乙方有使用權，但應負保管之責，如有破壞應負責修復並不得擅自增添設備，但經甲方允許者不在此限。<br>
							<br> 四、租用期間乙方對房屋任何部份如有損失應負回復原狀（或照價賠償）之責，交還房屋時如有發現有損害時亦同。<br>
							<br>
							五、乙方承租之房間止限於乙方及其家族居住不得引進外人居住，並應注重公共道德及衛生，違者甲方得隨時終止租約，請求乙方遷出，乙方不得異議。<br>
							<br> 上列各項條款均經雙方自願決不反悔，恐口無憑特立本契約二紙各執一份切實履行。<br>
						...
						<div style="font-size: 20px;" class="text-center">
							立契約書人 甲 方：<b>${memVO.mem_name}</b><br> 身分證字號：<b><input
								type="text" name="lan_idnumber" value="${eleConVO.lan_idnumber}"
								class="btn btn-light" placeholder="輸入房東身分證字號" /></b><br>
							...
							乙 方：<b><p id="mem_name2"></p></b><br> 身份證字號：<b><input
								type="text" name="mem_idnumber" value="${eleConVO.mem_idnumber}"
								class="btn btn-light" placeholder="輸入房客身份證字號" /></b><br> 簽約日：<b><input
								type="text" name="ele_singdate" id="ele_singdate"
								value="${eleConVO.ele_singdate}" class="btn btn-light" /></b><br>
						...
						<input type="hidden" name="action" value="insert"> <input
							type="hidden" name="con_id" value="${conVO.con_id}" /> <input
							type="hidden" name="ele_con_status" value="s1"> <input
							type="hidden" name="bill_paymenttype"
							value="${Bill_PaymentTypeMap['p1'].type_no}" /> <input
							type="hidden" name="lan_id"
							value="${lanSvc.getOneLanByMemId(memVO.mem_id).lan_id}" />
