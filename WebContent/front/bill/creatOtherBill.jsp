...
<%
	...
	pageContext.setAttribute("bill_id", bill_id);
%>
...
<%-- 										<p style="color:#3498DB" class="text-center">${memSvc.getOneMem(eleConVO.mem_id).mem_name}</p> --%>
										<p style="color:#3498DB" class="text-center">${memSvc.getOneMem(eleConSvc.getOneEC(billSvc.getOneB(bill_id).ele_con_id).mem_id).mem_name}</p>
									</div>
			
									<div>
										<b ><p>房屋地址</p></b>
										<p style="color:#3498DB">${houSvc.getOneHouse(eleConSvc.getOneEC(billSvc.getOneB(bill_id).ele_con_id).hou_id).hou_address}</p>
									</div>
			
									<div>
										<b><p>第一期繳交費用(租金+押金)</p></b>
										<p style="color:#3498DB">${eleConSvc.getOneEC(billSvc.getOneB(bill_id).ele_con_id).ele_rent_money}</p>
									</div>
									...
									<div>
										...
										<c:forEach var="Bill_PaymentType" items="${Bill_PaymentTypeMap}">
											<c:if
												test="${Bill_PaymentType.key eq eleConSvc.getOneEC(billSvc.getOneB(bill_id).ele_con_id).bill_paymenttype}">
								...
								<input type="hidden" name="action" value="payOtherBill">
								
								<input type="hidden" name="bill_id" value="${bill_id}"> 
<%-- 								<input type="hidden" name="ele_con_id" value="${billSvc.getOneB(bill_id).ele_con_id}">  --%>
<%-- 								<input type="hidden" name="bill_pay" value="${eleConSvc.getOneEC(billSvc.getOneB(bill_id).ele_con_id).ele_rent_money}"> --%>
									
<%-- 								<input type="hidden" name="bill_date" value="${billSvc.getOneB(bill_id).bill_date}"> --%>
<%-- 								<input type="hidden" name="bill_producetime" value="${billSvc.getOneB(bill_id).bill_producetime}">  --%>
<%-- 								<input type="hidden" name="bill_status" value="${billSvc.getOneB(bill_id).bill_status">  --%>
<!-- 								<input type="hidden" name="bill_paymethod" value="VISACard">  -->
<%-- 								<input type="hidden" name="bill_paymenttype" value="${eleConSvc.getOneEC(billSvc.getOneB(bill_id).ele_con_id).bill_paymenttype}">  --%>
