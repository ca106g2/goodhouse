...
<%
    ...
    List<RentMessVO> list = rentMessSvc.getAll();
...
<html>
<head>
<title>所有租屋訊息資料 - listAllRentMess.jsp</title>
...
<table id="table-1">
	<tr><td>
		 <h3>所有租屋資訊 - listAllRentMess.jsp</h3>
...
<table>
	...
	<c:forEach var="rentMessVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
		...
		<tr>
			...
			<td>${rentMessVO.hou_id}</td>
			<td>${rentMessVO.mem_id}</td>
			<td>${rentMessVO.lan_id}</td>
			...
			<td>
			  <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/back/rentMess/rentMess.do" style="margin-bottom: 0px;">
