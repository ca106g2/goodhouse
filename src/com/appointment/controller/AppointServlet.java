	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req
							.getRequestDispatcher("/appoint/select_page.jsp");
				...
				String url = "/appoint/listOneAppoint.jsp";
		...
		if ("getOne_For_Update".equals(action)) { // 來自listAllAppoint.jsp的請求
			...
			try {
				...
				/***************************3.查詢完成,準備轉交(Send the Success view)************/
				req.setAttribute("appointVO", appointVO);         // 資料庫取出的appointVO物件,存入req
				String url = "/appoint/update_appoint_input.jsp";
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/appoint/listAllAppoint.jsp");
				failureView.forward(req, res);
			}
		}
		...
		if ("update".equals(action)) { // 來自update_appoint_input.jsp的請求
			...
			try {
				...
				String app_remindsee = req.getParameter("app_remindsee");
				String app_remindseeReg = "^[A][0-9]{1}$";
				if (app_remindsee == null || app_remindsee.trim().length() == 0) {
					errorMsgs.add("預約時段: 請勿空白");
				} else if(!app_remindsee.trim().matches(app_remindseeReg)) { //以下練習正則(規)表示式(regular-expression)
					errorMsgs.add("預約時段: 只能是英文字母A開頭和數字 , 且總長度必需是2");
	            }
				...
				appointVO.setApp_remindsee(app_remindsee);

				// Send the use back to the form, if there were errors
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("appointVO", appointVO); // 含有輸入格式錯誤的appointVO物件,也存入req
					RequestDispatcher failureView = req
							.getRequestDispatcher("/appoint/update_appoint_input.jsp");
					failureView.forward(req, res);
					return; //程式中斷
				}
				...
				/***************************2.開始修改資料*****************************************/
				AppointService appointSvc = new AppointService();
				appointVO = appointSvc.updateAppoint(appoint_id, mem_id, lan_id, hou_id, hou_app_time, hou_app_date, app_status, app_remindsee);
        ...
        if ("insert".equals(action)) { // 來自addAppoint.jsp的請求  
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("appointVO", appointVO); // 含有輸入格式錯誤的AppointVO物件,也存入req
					RequestDispatcher failureView = req
							.getRequestDispatcher("/appoint/addAppoint.jsp");
					failureView.forward(req, res);
					return;
				}
				...
				/***************************2.開始新增資料***************************************/
				AppointService appointSvc = new AppointService();
				appointVO = appointSvc.addAppoint(mem_id, lan_id, hou_id, hou_app_time, hou_app_date, app_status, app_remindsee);
				
				/***************************3.新增完成,準備轉交(Send the Success view)***********/
				String url = "/appoint/listAllAppoint.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url); // 新增成功後轉交listAllRentMess.jsp
				successView.forward(req, res);				
