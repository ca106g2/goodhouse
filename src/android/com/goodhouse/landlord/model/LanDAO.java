...
public class LanDAO implements LanDAO_interface {
	...
	private static final String INSERT = "INSERT INTO LANDLORD VALUES ('L'||LPAD(to_char(LAN_SEQ.NEXTVAL), 9, '0'),?,?,?,?)";
	private static final String GET_ALL = "SELECT * FROM LANDLORD order by LAN_ID ";
	private static final String GET_ONE = "SELECT * FROM LANDLORD where LAN_ID=? ";
	private static final String DELETE = "DELETE FROM LANDLORD where LAN_ID=?";
	private static final String UPDATE = "UPDATE LANDLORD set MEM_ID=?,LAN_RECEIPT=?,LAN_ACCOUNT=?,LAN_ACCOUNTSTATUS=? where LAN_ID=?";
	private static final String GET_ONE_BY_MEM_ID = "SELECT * FROM LANDLORD where MEM_ID=?";
	...
	public void update(LanVO lanVo) {
		...
		try {
			...
			pstmt.setString(3, lanVo.getLan_account());
			pstmt.setString(4, lanVo.getLan_accountstatus());
			pstmt.setString(5, lanVo.getLan_id());
	...
	@Override
	public LanVO findByMemid(String mem_id) {
		LanVO lanVO = null;
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
